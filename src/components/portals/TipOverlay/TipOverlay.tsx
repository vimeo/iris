...
export interface TipOverlayProps {
  /**
...
export interface TipOverlayState {
  isShowing: boolean;
  isHovered: boolean;
}
...
const TipWrapperStyled = styled.div`
  opacity: 0;
...
export class TipOverlay extends Component<
  TipOverlayProps,
  TipOverlayState
> {
  static defaultProps = {
  ...
  constructor(props: TipOverlayProps) {
    super(props);
    this.state = {
      isShowing: false,
      isHovered: false,
  ...
  state: TipOverlayState;
  props: TipOverlayProps;
  overlay: any;
  menu: any;
    ...
    const tooltipComponent = (
      ...
      >
        {state => (
          ...
          >
            <Tip breakWords={breakWords}>{tooltipText as string}</Tip>
          </TipWrapperStyled>
        )}
      </Transition>
    );
