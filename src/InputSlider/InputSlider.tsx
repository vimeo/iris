...
const sliderThumbStyles = props => {
    return `
        ...
        background: ${getSliderThemeColors(props.format).handleBackground};
        pointer-events: auto;
        transform: translateX(
            ${rem(InputSliderStyleSettings.dimensions.gradientOffset)}
        );`;
};
...
const InputSliderStyled = styled<InputSliderStyledProps, 'div'>('div')`
    ...
    input[type='range'] {
        ...
        left: 0;
        top: 0;
        height: ${rem(InputSliderStyleSettings.dimensions.sliderTrackHeight)};
        width: 100%;
        outline: none;
        margin: 0;
        appearance: none;
        ...
        &:nth-child(2) {
            z-index: ${props => props.zIndexOverride + sliderZindex.endSlider};
            background: none;
        }
    }
`;
...
const BubbleWrapper = styled<BubbleWrapperStyleProps, 'div'>('div')`
    ...
    display: inline-block;
    width: 100%;
    z-index: ${props => props.zIndexOverride + sliderZindex.bubbleWrapper};
...
class InputSlider extends React.Component<any, any> {
    ...
    private onSliderValueChange = selectedInput => {
        ...
        this.setSliderBackground(this.startRange.value, this.endRange.value);
