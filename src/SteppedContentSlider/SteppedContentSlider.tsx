...
const ViewContainerStyled = styled.div<{
  ...
  ${props =>
    props.slideIsBiggerThanViewport ? `overflow-x: scroll;` : ''};
`;
...
> {
  ...
  componentDidUpdate(_, prevState: SteppedContentSliderState) {
    ...
    if (
      !this.state.initialPositionSet &&
      this.state.slideCountPerStep
    ) {
  ...
  buildSlides = (children = this.props.children) => {
    ...
    const formattedSlides = React.Children.map(
      children,
      (child, i) => {
  ...
  private _lastSlideVisible = (slide: number): boolean => {
    return (
      slide + this.state.slideCountPerStep >= this.state.slideCount
    );
  };
    ...
    if (
      ...
      const slideCountPerStep =
        this.props.stepMode === 'group'
          ...
          : 1;
      const slideIsBiggerThanViewport =
