...
const buildSizeCSS = props => sizes.map((size) => {
    const sizeSpan = props[`${size}Span`];
    const sizeFixed = props[`${size}Fixed`];

    const sizeOffset = size === 'xs'
        ? props.offset
        : props[`${size}Offset`];

    const validProp = prop => prop || prop === 0;

    const width = validProp(sizeFixed)
        ? BREAKPOINTS[size]
        : 100;
    
    return [
        validProp(sizeSpan)
            ? buildSpanMQ(sizeSpan, size, width)
            : null,
        validProp(sizeOffset)
            ?  buildOffset(sizeOffset, size)
            : null
    ];
});
...
const buildOffset = (offset, mq = null) => {
    ...
    return mq
        ? mediaQuery[mq]`${marginCSS}`
        : marginCSS;
};

export const GridColStyled = styled<GridColProps, any>(GridColVariableElement)`
